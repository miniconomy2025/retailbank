...
public class LoanService(ILedgerRepository ledgerRepository, IOptions<LoanOptions> options) : ILoanService
{
    public async Task<UInt128> CreateLoanAccount(UInt128 debitAccountNumber, ulong loanAmount)
    {
        ...
        await ledgerRepository.Transfer(new LedgerTransfer(ID.Create(), accountNumber, debitAccountNumber, loanAmount, 0, TransferType.Transfer));
